...
This documents how to use **tk**, a git-backed ticket tracker. Tickets are
markdown files with YAML frontmatter stored in `.tickets/` inside a project.

Run `tk help` for the full command reference.

## Task Workspaces

When working in a task workspace (e.g. under `~/.workspaces`), use `tk current`
to print the full path to the current in-progress ticket file:

```bash
tk current    # Outputs /absolute/path/to/project/.tickets/<id>.md
```

`tk current` expects exactly one root ticket with status `in_progress`, then
follows a single chain of `in_progress` child tickets to the deepest one.
...
4. Update the ticket file as you work (analysis, progress notes, checklists)
...
tk create "Title" --external-ref gh-123
tk create "Title" --parent <id>
```
...
Tickets are markdown files with YAML frontmatter. Edit directly with
`tk edit <id>` or your editor.
...
- **Description**: Write directly below the `# Title` heading, before any `##`
  sections
- **Notes**: Prefer `tk add-note` so notes are timestamped consistently
- **Progress/Analysis**: Add custom sections like `## Analysis`, `## Progress`,
  `## Summary of Changes` as needed
